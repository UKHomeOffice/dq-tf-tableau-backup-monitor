...
def send_message_to_slack(text):
    ...
    try:
        post = {
            "text": ":fire: :sad_parrot: *EXTERNAL TABLEAU BACKUP PROCESS DID NOT COMPLETE* Files are not regularly arriving in *DATA ARCHIVE* :sad_parrot: :fire:",
            "attachments": [
                {
                    "text": "{0}".format(text),
                    "color": "#B22222",
    ...
    try:
        ...
        try:
            ...
            if objs:
                ...
                if x_mins > obj_utc:
                    LOGGER.info('Please investigate External Tableau Backup Uploads. No backups uploaded for the last {0} minutes'.format(threashold_min))
                    send_message_to_slack('Please investigate * External Tableau Backups*! No backups uploaded for the last {0} minutes. Last backup {1} was uploaded on {2} '.format(threashold_min, obj_name, obj_utc))
                else:
                ...
                send_message_to_slack('Please investigate External Tableau Backup Uploads! No backups uploaded for the last {0}'.format(x_mins.strftime('%Y-%m-%d')))
